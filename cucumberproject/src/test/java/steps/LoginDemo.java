package steps;

import io.cucumber.java.en.*;

public class LoginDemo 
{
	
	@Given("the user is on the login page")
	public void the_user_is_on_the_login_page() {
	    
	}

	@When("the user enters {string} into the username field")
	public void the_user_enters_into_the_username_field(String string) {
	   
	}

	@When("the user enters {string} into the password field")
	public void the_user_enters_into_the_password_field(String string) {
	   
	}

	@When("the user clicks the Submit button")
	public void the_user_clicks_the_submit_button() {
	    
	}

	@Then("the new page URL should contain {string}")
	public void the_new_page_url_should_contain(String string) {
	   
	}

	@Then("the new page should display text containing {string} or {string}")
	public void the_new_page_should_display_text_containing_or(String string, String string2) {
	   
	}

	@Then("the {string} button should be visible on the new page")
	public void the_button_should_be_visible_on_the_new_page(String string) {
	   
	}

	@Then("an error message should be displayed")
	public void an_error_message_should_be_displayed() {
	    
	}

	@Then("the error message text should be {string}")
	public void the_error_message_text_should_be(String string) {
	    
	}


}
